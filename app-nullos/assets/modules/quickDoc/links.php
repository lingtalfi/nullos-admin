<?php

$store = [
    'found' => [
        '/README.md' => [
            [
                'kif',
                'https://github.com/lingtalfi/kif',
            ],
            [
                'tutorials section',
                'tutorials',
            ],
            [
                'Init writer',
                'modules/boot-module/init-writer-page.md',
            ],
            [
                'Reset',
                'modules/boot-module/reset-page.md',
            ],
            [
                'Crud Generators',
                'modules/crud-module/crud-generators-page.md',
            ],
            [
                'Execute SQL',
                'modules/sqltools-module/execute-sql-page.md',
            ],
            [
                'QuickDoc',
                'modules/quickdoc-module.md',
            ],
            [
                'IconsViewer',
                'modules/iconsviewer-module.md',
            ],
            [
                'Linguist',
                'modules/linguist-module.md',
            ],
            [
                'NullosInfo',
                'modules/nullosinfo-module.md',
            ],
            [
                'kif application',
                'https://github.com/lingtalfi/kif',
            ],
            [
                'Privilege framework',
                'https://github.com/lingtalfi/Privilege',
            ],
            [
                'getting started with nullos admin',
                'tutorials/getting-started-with-nullos-admin.md',
            ],
        ],
        '/bonus/developer-guidelines/developer-guidelines.md' => [
            [
                'Module guidelines',
                'bonus/developer-guidelines/module-guidelines.md',
            ],
        ],
        '/bonus/developer-guidelines/module-guidelines.md' => [
            [
                'developers guidelines',
                'bonus/developer-guidelines/developer-guidelines.md',
            ],
            [
                'Boot module',
                'modules/boot-module.md',
            ],
            [
                'list of log calls',
                'core/logger/list-of-log-calls.md',
            ],
        ],
        '/core/layout/tabby.md' => [
            [
                'nullos Layout object',
                'core/layout.md',
            ],
            [
                'Linguist module',
                'modules/linguist-module.md',
            ],
            [
                'QuickDoc module',
                'modules/quickdoc-module.md',
            ],
        ],
        '/core/layout.md' => [
            [
                'kif framework',
                'https://github.com/lingtalfi/kif',
            ],
        ],
        '/core/logger/list-of-log-calls.md' => [
            [
                'Nullos info module',
                'modules/nullosinfo-module.md',
            ],
        ],
        '/core/logger.md' => [
            [
                'ApplicationLogModule',
                'modules/applicationlog-module.md',
            ],
            [
                'List of log calls',
                'core/logger/list-of-log-calls.md',
            ],
            [
                'Nullos aliases',
                'bonus/nullos-aliases.md',
            ],
        ],
        '/core/nullos-account.md' => [
            [
                'configuring the nullos accounts',
                'modules/authentication-module/configuring-nullos-accounts.md',
            ],
        ],
        '/core/nullos-permissions.md' => [
            [
                'linguist module',
                'modules/linguist-module.md',
            ],
            [
                'crud module',
                'modules/crud-module.md',
            ],
            [
                'application log module',
                'modules/applicationlog-module.md',
            ],
        ],
        '/core/privilege/list-of-all-privileges.md' => [
            [
                'Privilege framework\'s namespace convention',
                'https://github.com/lingtalfi/Privilege#namespace-convention-and-wildcards',
            ],
            [
                'NullosInfo module',
                'modules/nullosinfo-module.md',
            ],
            [
                'modules guidelines',
                'bonus/developer-guidelines/module-guidelines.md',
            ],
        ],
        '/core/privilege/privilege-profile.md' => [
            [
                'nullos account',
                'core/nullos-account.md',
            ],
            [
                'Configuring the Privilege profiles',
                'configuring-privilege-profiles.md',
            ],
            [
                'List of all privileges',
                'core/privilege/list-of-all-privileges.md',
            ],
        ],
        '/modules/applicationlog-module/configuring-log-path.md' => [
            [
                'ApplicationLog module',
                'modules/applicationlog-module.md',
            ],
        ],
        '/modules/applicationlog-module.md' => [
            [
                'Logger',
                'core/logger.md',
            ],
            [
                'Configuring the log path',
                'modules/applicationlog-module/configuring-log-path.md',
            ],
        ],
        '/modules/authentication-module/login-page.md' => [
            [
                'Authentication module',
                'modules/authentication-module.md',
            ],
        ],
        '/modules/authentication-module.md' => [
            [
                'Configuring the nullos accounts',
                'modules/authentication-module/configuring-nullos-accounts.md',
            ],
            [
                'The login page',
                'modules/authentication-module/login-page.md',
            ],
        ],
        '/modules/boot-module/init-writer-page.md' => [
            [
                'lang module',
                'modules/lang-module.md',
            ],
        ],
        '/modules/boot-module/reset-page.md' => [
            [
                'left menu preferences',
                'modules/crud-module/left-menu-preferences.md',
            ],
            [
                'crud files',
                'modules/crud-module/crud-files.md',
            ],
            [
                'crud files preferences',
                'modules/crud-module/crud-files-preferences.md',
            ],
        ],
        '/modules/boot-module.md' => [
            [
                'the "Init writer" page',
                'modules/boot-module/init-writer-page.md',
            ],
            [
                'the "Reset" page',
                'modules/boot-module/reset-page.md',
            ],
        ],
        '/modules/crud-module/crud-files-preferences.md' => [
            [
                'Crud module',
                'modules/crud-module.md',
            ],
            [
                'Crud Generator page',
                'modules/crud-module/crud-generators-page.md',
            ],
        ],
        '/modules/crud-module/crud-files.md' => [
            [
                'crud page',
                'modules/crud-module/crud-page.md',
            ],
            [
                '"Crud Generators" page',
                'modules/crud-module/crud-generators-page.md',
            ],
            [
                'configure a list',
                'tutorials/configure-a-list.md',
            ],
        ],
        '/modules/crud-module/crud-page.md' => [
            [
                'ric values',
                'modules/crud-module/ric.md',
            ],
            [
                'crud file',
                'modules/crud-module/crud-files.md',
            ],
            [
                '"Reset" page',
                'modules/boot-module/reset-page.md',
            ],
        ],
        '/modules/crud-module/left-menu-preferences.md' => [
            [
                'crud generators page',
                'modules/crud-module/crud-generators-page.md',
            ],
        ],
        '/modules/crud-module.md' => [
            [
                'The "Crud Generators" page',
                'modules/crud-module/crud-generators-page.md',
            ],
            [
                'The crud files preferences',
                'modules/crud-module/crud-files-preferences.md',
            ],
            [
                'The crud files',
                'modules/crud-module/crud-files.md',
            ],
            [
                'Left menu preferences',
                'modules/crud-module/left-menu-preferences.md',
            ],
            [
                'Crud page',
                'modules/crud-module/crud-page.md',
            ],
            [
                'Configure a list',
                'tutorials/configure-a-list.md',
            ],
        ],
        '/modules/lang-module/translation-files.md' => [
            [
                'Lang module',
                'modules/lang-module.md',
            ],
            [
                'translation function',
                'modules/lang-module/translation-function.md',
            ],
        ],
        '/modules/lang-module.md' => [
            [
                'Translation files',
                'modules/lang-module/translation-files.md',
            ],
            [
                'The translation function',
                'modules/lang-module/translation-function.md',
            ],
        ],
        '/modules/linguist-module.md' => [
            [
                'translation files',
                'modules/lang-module/translation-files.md',
            ],
            [
                'tabby tabs',
                'core/layout/tabby.md',
            ],
            [
                'translations files',
                'modules/lang-module/translation-files.md',
            ],
        ],
        '/modules/quickdoc-module.md' => [
            [
                'README.md',
                'README.md',
            ],
            [
                'tabby layout',
                'core/layout/tabby.md',
            ],
        ],
        '/modules/sqltools-module.md' => [
            [
                'The "Execute SQL" page',
                'modules/sqltools-module/execute-sql-page.md',
            ],
        ],
        '/tutorials/configure-a-list.md' => [
            [
                'Install the "oui" database tutorial',
                'tutorials/install-oui-database.md',
            ],
            [
                'ric',
                'modules/crud-module/ric.md',
            ],
        ],
        '/tutorials/getting-started-with-nullos-admin.md' => [
            [
                'nullos permissions',
                'core/nullos-permissions.md',
            ],
            [
                'oui-schema',
                'assets/oui-schema.png',
            ],
            [
                'SqlTools module',
                'modules/sqltools-module.md',
            ],
        ],
        '/tutorials/install-oui-database.md' => [
            [
                'oui.sql',
                '/fixtures/oui.sql',
            ],
            [
                'oui schema',
                'assets/oui-schema.png',
            ],
            [
                'Getting started with nullos',
                'tutorials/getting-started-with-nullos-admin.md',
            ],
        ],
    ],
    'unfound' => [
        '/modules/crud-module/crud-files.md' => [
            [
                'configure a form',
            ],
        ],
        '/modules/quickdoc-module.md' => [
            [
                'link',
            ],
        ],
        '/tutorials/configure-a-list.md' => [
            [
                'Configure a form',
            ],
        ],
    ],
];
